...
### 13. [4주차 작업 결과](#4주차-작업-결과)
### 14. [5주차 작업 결과](#5주차-작업-결과)
### 15. [6주차 작업 결과](#6주차-작업-결과)
### 16. [7주차 작업 결과(2학기 마무리)](#7주차-작업-결과)
...
### 6주 요구사항
- ~~게임을 실행시 메인화면이 출력되며 메인화면에는 시작, 환경 설정, 종료 버튼이 있다.~~
- ~~시작화면을 누르면 인트로 화면으로 전환된다.~~
- ~~환경 설정 버튼을 누르면 음량을 조절할 수 있는 슬라이더가 나온다.~~
- ~~종료 버튼을 누를 시 게임이 종료 된다.~~
- ~~인트로에서는 간략한 스토리와 조작법이 나온다.~~
- ~~인트로화면에서 간략한 스토리와 조작법을 A/S 버튼으로 스킵 가능하다.~~
- ~~인트로화면에서 S버튼을 누르면 인게임 씬으로 이동한다.~~
- ~~인게임 씬은 1인칭으로 설계된 플레이어가 마우스, 키보드를 통해 시점을 이동하고 위치를 이동한다.~~
- ~~어두운 공간에서 시야를 확보할 수 있는 플레이어가 들고다니는 횃불 구현.~~
- ~~아이템이라는 요소가 추가되며 아이템은 유저가 인벤토리에 저장할 수 있으며 아이템은 열쇠 및 퍼즐이 해당한다.~~
- ~~적절한 상황에 맞는 소리를 추가한다. (발걸음, 달리기, 아이템획득, 문열림)~~
- ~~메인문 보석 인식 추가 (중간문 해금)~~
- ~~시작할때마다 다른 방으로 시작하도록 구현 (랜덤)~~ 

### 1년 요구사항

- 유저는 문, 통신기, 보드게임판 등과 접촉시 상호작용 여부를 묻는 텍스트를 출력하고 t버튼을 누를 시 물체와 상호작용한다.
- 미니게임이란 게임 진행중, 혹은 게임의 마지막 구간에서 설치된 미니 게임 보드판을 유저가 상호작용할 수 있는 물체이다. 
- 미니게임을 실행시 미니게임 씬으로 전환되며 미니 게임을 클리어시 다시 인게임 씬으로 돌아온다. 
- 인게임과 미니게임은 제한 시간이 존재한다.
- 미니게임은 숫자 퍼즐 형식의 게임이 될것이다.
- 점멸 스킬이 추가 된다.
- 유저가 사용하는 스킬을 사용하기 위한 에너지를 추가한다.
- 문, 통신기 등의 사물을 사용하기 위한 전력의 개념이 추가된다.
- 전력을 충전하기 위한 배터리 아이템이 추가된다.
- 챕터2 부터 유저를 방해하는 적군과 함정이라는 요소가 설치된다.
- 챕터2 부터는 체력이 설정되며 유저는 적군과 함정에 계속해서 피해를 입으면 게임 오버 된다.
- 챕터2 부터는 상호작용시 유저가 게임 진행도중 여러가지 도움을 텍스트로 줄 통신기 개념이 추가된다.
...
2. 플레이어가 움직일 수 있는 기본적인 맵을 생성함. (100%) 
...
1. 맵 디자인  (시작방, 막힌 문) (100%)
2. 오브젝트 배치 (100%)
3. 인벤토리 제작 (100%)
4. 플레이와 와 오브젝트 간의 상호작용 구현(상호작용후 인벤토리 미구현) (70%) 4주차 완료
...
3. 문 오브젝트 상호작용 설계 (일반적으로 열리는 문, 열리는데 조건이 필요한 문) (20%) 5주차 완료
4. 맵에 있는 방마다 다른 오브젝트를 설치함. (30%) 6주차 완료
...
1. 상황에 맞는 사운드 넣기 (발걸음, 문열림, 아이템획득, bgm) 25% (발걸음만 추가) 7주차 완료
2. 맵구체화 (중문을 여는데 필요한 오브젝트 및 다양한 기물들 추가) 60% (다양한 기물들 추가못함) 7주차 완료
...
1. 메인 메뉴, 인트로 화면을 다듬기. (100%)
2. 게임의 배경 음악 추가, 상호작용시 소리 추가. (0%) 
3. 인트로 화면에서 게임에 상세한 스토리를 추가. (50%) 7주차 완료
4. 맵에 가구를 배치하고 오브젝트들을 숨겨놓는 작업.  (20%) 7주차 완료
5. 실행시마다 랜덤 방으로 생성 (100%) 

-구현 영상

-랜덤 방 구현
<video width="100%" height="100%" controls="controls">
  <source src="./video/6week/랜덤방시작.mp4" type="video/mp4">
</video>

# [7주차 작업 결과]
  
- 요구사항 
1. 지금 까지의 작업물 보충 및 마무리 다듬기

- 작업 내용

1. 맵에 가구를 배치하고 오브젝트들을 숨겨놓는 작업. (100%)
2. 여러가지 상황의 소리 추가. (달리기, 문열림, 아이템획득) (100%) 
3. 인트로 화면에서 게임에 상세한 스토리를 추가. (100%)
4. 첫번째 제외 다른방 구성물 추가 (100%)
