...
|속성|영문 명칭|설명|비고|
|:----:|:----:|:----:|:----:|
|빛|light|어두운 공간속을 밝혀주는 값||

### 2) 오브젝트 이름 : 배터리

|속성|영문 명칭|설명|비고|
|:----:|:----:|:----:|:----:|
|전기|electric|발전기에 동력원이 되는 파라미터||
...
|:----:|:----:|:----:|:----:|
|전기|electric|발전기에 동력원이 되는 파라미터||
|충전량|storage|축전되는 전기용량을 나타내주는 값||
|최대용량|maxstorage|축전 가능한 최대치로 한계치를 의미||

### 4) 오브젝트 이름 : 승강기

|속성|영문 명칭|설명|비고|
|:----:|:----:|:----:|:----:|
|상태|L_state|미니게임 활성화 가능 여부를 판단한다.(전력)||
|활성화|L_activate|미니게임 화면으로 전환된다.||

### 5) 오브젝트 이름 : 통신기

|속성|영문 명칭|설명|비고|
|:----:|:----:|:----:|:----:|
|상태|T_state|통신이 가능한 상태인지 확인한다. (전력)||
|활성화|T_activate|통신기를 연결한다.||
|힌트1|T_MiniHint|미니게임에 대한 힌트를 얻는다.||
|힌트2|T_GameHint|같은 층내 배터리가 숨겨져있을 위치를 알려준다.||

### 6) 오브젝트 이름 : 미니게임

|속성|영문 명칭|설명|비고|
|:----:|:----:|:----:|:----:|
|타이머|Timer|게임플레이를 진행할수있는 제한시간||
||| 추가 예정 ||
...
|속성|영문명칭|설명|비고|
|:----:|:----:|:----:|:----:|
|이동속도|speed|플레이어가 이동시 속도값.||
|에너지|Energy|대쉬 및 벽통과를 하는데 사용하는 값.  </br>  스킬이 지속되지 않을시 천천히 차오른다.  ||
...
6.  조작법은 키보드에 WASD 버튼으로 상하좌우로 움직이며 T버튼으로 물체와 상호작용, C/V 버튼으로 기술을 사용한다. 인게임 진행중 ESC 버튼을 누     를 시 게임을 정지 하고 환경 설정과 게임 종료 버튼이 있는 팝업창이 나옴.
