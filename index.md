...
### 6주 요구사항
~~- 게임을 실행시 메인화면이 출력되며 메인화면에는 시작, 환경 설정, 종료 버튼이 있다.~~<br>
~~- 시작화면을 누르면 인트로 화면으로 전환된다.~~ <br>
...
- 한 게임안에 여러개의 챕터가 존재하며 챕터가 진행될수록 컨텐츠가 늘어난다.
- 늘어나는 컨텐츠에는 다른 장르의 미니게임과 스킬등이 있다.
- 스킬은 대쉬/점멸 등이 스킬이 존재한다.
...
- 아이템이라는 요소가 추가되며 아이템은 유저가 인벤토리에 저장할 수 있으며 아이템은 열쇠, 전력(배터리)가 해당한다. 
...
- 요구사항
 ~~- 게임을 실행하면 게임 시작 버튼, 환경 설정, 종료 버튼이 있다.~~
 ~~- 시작화면을 누르면 인트로가 나온다.~~
 ~~- 환경 설정 버튼을 누르면 소리 On/off 혹은 조절이 나온다.~~
 ~~- 종료 버튼을 누르면 게임이 종료된다.~~
 ~~- 인트로에서는 간략한 스토리와 조작법이 나온다.~~

- 작업 내용
 - 게임 시작 버튼을 누를 시 두번째 신(인트로)로 넘어 간다. (100%)
 - 환경 설정 버튼을 누르면 팝업창이 나오고 음량을 조절하는 드래그창이 나온다 (100%)
 - 종료 버튼을 누르면 게임 종료가 된다.
 - 인트로에서 간략한 스토리 텍스트가 나오고 10초가 지나면 조작법을 보여주는 텍스트로 전환 한다. 인트로를 생략하는 스킵 버튼도 추가 한다. (100%)
...
- 요구사항
~~- 메인 메뉴 - 인트로 - 인게임으로 진행하도록 인게임 신을 따로 제작.~~
~~- 인게임에서 플레이어가 활동할 수 있는 기본적인 맵 생성.~~
~~- 플레이어를 생성하고 플레이어에 기본적인 값 부여(속도, 시점)~~
~~- 플레이어가 키 값(W, A, S, D)을 받으면 움직이도록 설정.~~

- 작업 내용

- 게임 시작 버튼 - 인트로 화면 - 인게임 신으로 구성하게 만듬. (100%)
- 플레이어가 움직일 수 있는 기본적인 맵을 생성함. (움직임 테스트용 공간 게임맵은 추후 제작예정) (50%) 
- 플레이어를 생성하고 플레이어가 가진 기본값을 설정 (유저 시점, 속도, 키 이벤트) (100%)
- 어두운 환경속 시야 확보를 위한 횃불 생성. (100%)
